...
const preload_mods = {
    console: require('./appenders/console')
}
...
class Configuration {
  ...
  loadAppenderModule(type) {
    return preload_mods[type] ||
      this.tryLoading(`./appenders/${type}`) ||
  ...
  createAppender(name, config) {
    ...
    if (this.disableClustering || cluster.isMaster || (this.pm2 && process.env[this.pm2InstanceVar] === '0')) {
      debug(`disableClustering ? ${this.disableClustering}`);
      if ( ! this.disableClustering) {
